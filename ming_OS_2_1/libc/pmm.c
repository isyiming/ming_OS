  ...
  *   这个页表管理的代码是Hurley的原创，当然我移植过来就变得很不一样了。
  *   我在cfenollosa的基础上继续实现系统的时候，原本是看JamesM的文章。
  ...
  pmm_stack.phy_page_count=0;
	...
	for (uint32_t page_addr = PMM_START_ADDR; page_addr < PMM_END_ADDR; page_addr+=PMM_PAGE_SIZE) {
      ...
      // pmm_stack.PhysicalAddr[pmm_stack.phy_page_count]=page_addr;//入栈
			...
			// kprint("  page addr      : "); print_hex(page_addr);      kprint("   ");  kprint("\n");
	}
...
{
	ASSERT("out of pmm_stack stack",pmm_stack.phy_page_count != PAGE_MAX_SIZE);

	pmm_stack.PhysicalAddr[++pmm_stack.phy_page_count] = p;
}
